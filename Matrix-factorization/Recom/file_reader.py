...
def matrix_factorization(pd_matrix):
    ...
    for step in range(steps):
        for i in range(number_of_users):
            for j in range(number_of_movies):
                if(numpy_matrix[i][j]) > 0:
                    ...
                    error = numpy_matrix[i][j] - np.dot(user_features[i, :], movie_features[:, j])

                    for k in range(features):
                        user_features[i][k] = user_features[i][k] + alpha * (2 * error * movie_features[k][j]
                                                                             - beta * user_features[i][k])
                        movie_features[k][j] = movie_features[k][j] + alpha * (2 * error * user_features[i][k]
        ...
        for i in range(number_of_users):
            for j in range(number_of_movies):
                if (numpy_matrix[i][j]) > 0:
                    mse = mse + pow(numpy_matrix[i][j] - np.dot(user_features[i, :], movie_features[:, j]), 2)

                    for k in range(features):
                        mse = (mse + (beta / 2) * (pow(user_features[i][k], 2) + pow(movie_features[k][j], 2)))
