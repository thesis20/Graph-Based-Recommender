...
class LightGCN():
    def load_data(self, training_size=0.8):
        ...
        v = full_data[['userId']]
        full_data = full_data[v.replace(
            v.apply(pd.Series.value_counts)).gt(10).all(1)]
    ...
    def train(self, embedding_size=64):
        user_item_matrix = torch.sparse.Tensor(
            (self.user_count, self.item_count), dtype=np.bool)
        ...
        adjacency_matrix = lil_matrix(
            (user_item_count, user_item_count), dtype=np.bool)

        for i in range(self.user_count):
            for j in range(self.item_count):
                # insert values of R into adjacency matrix
                adjacency_matrix[i, j +
                                 self.user_count] = user_item_matrix[i, j]
                # insert values of R_transposed into adjacency matrix
                adjacency_matrix[j + self.user_count,
                                 i] = user_item_matrix[i, j]

        embedding_matrix = lil_matrix(
            (user_item_count, embedding_size), dtype=np.float32)
        diagonal_matrix = lil_matrix(
            (user_item_count, user_item_count), dtype=np.int32)

        for i in range(user_item_count):
            print(adjacency_matrix[i].count_nonzero())
            diagonal_matrix[i, i] = adjacency_matrix[i].count_nonzero()

        embedding_matrix = ((diagonal_matrix**0.5)*adjacency_matrix *
                            (diagonal_matrix**0.5))*embedding_matrix
